...
func (t *Transformer) addToBatch(ctx context.Context, e *entry.Entry, source string, matches bool) {
	...
	if matches && !t.matchDetected(source) {
		t.matchMap[source] = true
	}
	...
	if (t.maxLogSize > 0 && int64(batch.recombined.Len()) > t.maxLogSize) ||
		...
		(!t.matchDetected(source) && t.maxUnmatchedBatchSize > 0 && batch.numEntries >= t.maxUnmatchedBatchSize) {
...
// matchDetected checks if match for source occurred
func (t *Transformer) matchDetected(source string) bool {
	match, ok := t.matchMap[source]
	if ok {
		return match
	}
	return false
